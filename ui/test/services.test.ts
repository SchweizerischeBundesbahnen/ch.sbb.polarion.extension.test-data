...
afterEach(() => {
  ...
  document.cookie = 'test-data-test=; path=/; max-age=0';
});

describe('cookies', () => {
  it('round-trips a value and returns null for a missing one', () => {
    expect(getCookie('test-data-test')).toBeNull();
    setCookie('test-data-test', 'hello world');
    expect(getCookie('test-data-test')).toBe('hello world');
  });
});
