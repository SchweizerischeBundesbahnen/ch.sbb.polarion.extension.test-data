...
export default defineConfig(({ command, mode }) => {
    ...
    return {
      ...
      server: {
        proxy: {
          ...
          // unauthenticated in Polarion (see the test-data-app web.xml), so the dev proxy can fetch
          // it without a session.
          '/polarion/test-data-app/ui/generic': {
            target: polarionUrl,
            changeOrigin: true,
          },
          // The extension's own webapp context: its REST API, which the About page reads.
          '/polarion/test-data/rest': {
            target: polarionUrl,
            changeOrigin: true,
          },
          '/polarion/rest': {
  ...
  return {
    ...
    base: '/polarion/test-data-app/ui/app/',
    build: {
      outDir: './dist/app',
      emptyOutDir: true,
    },
